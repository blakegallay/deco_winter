t rom zoom_predictions_4class import run_blob_classifier
...
for path in paths:
    ...
    cnn_data = [cnn_row['p_spot'][index], cnn_row['p_track'][index],
                cnn_row['p_worm'][index], cnn_row['x_coord'][index], cnn_row['y_coord'][index]]
    ...
    event_hillas_params = cnn_data[0:3] + hillas(
        [np.asarray(charge_x), np.asarray(charge_y), np.asarray(charges)]) + cnn_data[3:]
...
with open('./results/Spot.csv', 'w') as outfile:
    ...
    writer.writerow(["event_id", "p_spot", "p_track", "p_worm",
                     "width", "length", "azwidth", "x_coord", "y_coord"])
    for p in hillas_params:
        writer.writerow(p)

with open('./results/Spotstats.csv', 'w') as statsfile:
    writer = csv.writer(statsfile)
    writer.writerow(['variable', 'mean', 'stdev',
                     'min', 'Q1', 'median', 'Q3', 'max'])
    variables = {"width": widths, "length": lengths, "azwidth": azwidths}
    for name in variables:
        var = variables[name]
        writer.writerow([name, np.mean(var), np.std(var), np.min(
            var)] + list(np.percentile(var, [25, 50, 75])) + [np.max(var)])
