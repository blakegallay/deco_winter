...
cnn_paths = []
# Running unprocessed images through the CNN
...
if(len(cnn_paths) > 0):
	...
	df = run_blob_classifier(cnn_paths, 'out.csv', 4, weights_file=cnn_weights_file)
	
	for i in range(len(df.index)):
		cnn_data[str(df['event_id'][i])] = [df['image_file'][i], df['p_noise'][i], df['p_spot'][i], df['p_track'][i], df['p_worm'][i], df['x_coord'][i], df['y_coord'][i]]
...
for dir in os.listdir(source_dir):
	...
	with open(export_dir + dir + ".csv", 'w') as out:
		...
		writer.writerow(["event id", "width", "length", "miss", "dis", "azwidth"])
...
# cumulative stats
variables_all = {"width":all_widths, "length":all_lengths, "azwidth":all_azwidths}
stats["All"] = {}
for name in variables_all:
	var = variables_all[name]
	stats["All"][name] = [np.mean(var), np.std(var)] + [np.min(var)] + list(np.percentile(var, [25,50,75])) + [np.max(var)]
...
with open("./results/irradiation/stats.csv", 'w') as statsfile:
	...
	for category in stats:
		...
		writer.writerow(["", "mean", "stdev", "min", "Q1", "median", "Q3", "max"])
