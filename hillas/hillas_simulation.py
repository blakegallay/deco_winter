    ...
    for sample in os.listdir(source_dir + category):
        ...
        if(sample_name not in master_data[category]):
            ...
            with open(export_dir + category + "/" + sample_name + ".csv", 'w') as out:
                ...
                headings = ["event id", "p_spot", "p_track", "p_worm",
                            "x_coord", "y_coord", "width", "length", "azwidth"]
                ...
                if(len(arrs) > 0):
                    ...
                    for i in range(len(df.index)):
                        cnn_data[df['event_id'][i]] = [df['p_noise'][i], df['p_spot'][i],
                                                       df['p_track'][i], df['p_worm'][i], df['x_coord'][i], df['y_coord'][i]]
            ...
            if(len(arrs) > 0):
                master_data[category][sample_name] = [df['p_spot'].mean(), df['p_track'].mean(), df['p_worm'].mean(
                ), np.mean(widths), np.mean(lengths), np.mean(azwidths), df['x_coord'].mean(), df['y_coord'].mean()]

                with open(stats_dir + category + ".csv", 'w') as statsfile:
                    writer = csv.writer(statsfile)
                    headings = ["sample", "p_spot", "p_track", "p_worm",
